...
class StdinSequenceRunner:
    ...
    def enqueue_line(self, line: str):
        ...
        if low == "reset":
            if self.board_enabled["board_a"]:
                self.rc.board_a.reset_encoder()
            if self.board_enabled["board_b"]:
                self.rc.board_b.reset_encoder()
            self.log("[stdin] 已发送 reset（仅作用于启用板）")
            return

        parts = line.split()
        if len(parts) != 2:
            self.log("[stdin] 格式错误，应为：手指序列 时长序列，例如 23232 23222")
            return
        ...
        try:
            while not self.cmd_queue.empty():
                ...
                for fch, tch in zip(finger_seq, time_seq):
                    ...
                    ok = self.sleep_with_stop(duration)

                    self.safe_motor(finger, "rest")
                    self.log(f"[seq] {finger} -> rest")

                    if not ok:
                        break

                    if self.gap > 0:
                        ok = self.sleep_with_stop(self.gap)
                        if not ok:
                            break
    ...
    def run(self):
        ...
        try:
            ...
            try:
                if self.board_enabled["board_a"]:
                    self.rc.board_a.disconnect()
                if self.board_enabled["board_b"]:
                    self.rc.board_b.disconnect()
            except Exception:
                pass
